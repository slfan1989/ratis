...
public class TestNettyRpcProxy extends BaseTest {
  ...
  public void testOfferRollbackOnAlreadyClosed() throws Exception {
    ...
    try {
      ...
      // Close to force AlreadyClosedException on write.
      proxy.close();
      ...
      final Throwable thrown = Assertions.assertThrows(CompletionException.class, reply::join);
      final Throwable unwrapped = JavaUtils.unwrapCompletionException(thrown);
      Assertions.assertTrue(unwrapped instanceof AlreadyClosedException);

      // The replies queue must be empty after rollback.
      final Object connection = getField(proxy, "connection");
      final Queue<?> replies = getField(connection, "replies");
